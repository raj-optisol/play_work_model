  ...
  Foundation.libs.forms = {
    ...
    events: function () {
      ...
      $(window).on('keydown', function (e) {
              ...
              var dropdown = dropdowns.filter(".open");


              var inputindex = -1;
              var sel = focus;

              var dd = dropdowns.has(focus);
              if(dd.length>0)
                sel = getFirstPrevSibling(dd, 'select');


              if(dd.length > 0 && (!(inputindex = inputs.index(focus)) || inputindex <0))
              {
                e.preventDefault();
              }
              ...
              if (e.which === 9) {
                ...
                if (e.shiftKey)
                 ...
                 var nxt = $(inputs[inputs.index(sel)+dir]);
                 if(nxt.is("select"))
                 {
                   e.preventDefault();
                   nxt = nxt.next().find('.current')
                   nxt.focus();
                 }
                 else if (dd.length>0)
                    nxt.focus();
              ...
              if (dropdown.length > 0) {
                ...
                if (e.which === 38) {
                  ...
                  var current = dropdown.find('li.selected');
                  if(!current.length && sel)
                  {
                    if(sel[0])
                      current = dropdown.find('li').eq(sel[0].selectedIndex)
                  }
                  var next = current.next(':not(.disabled)');
      ...
      $(window).on('keyup', function (e) {
          ...
          var findex = -1;
          if ( (findex = dropdown.find('.current').index(focus)) >=0) {
            dropdown.find('.selector').eq(findex).focus().click();
          }
      });
