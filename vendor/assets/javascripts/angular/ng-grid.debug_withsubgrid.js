...
ngGridServices.factory('$domUtilityService',['$utilityService', function($utils) {
    ...
    domUtilityService.BuildStyles = function($scope, grid, digest) {
        ...
        if (!$style) {
            $style = $('#' + gridId);
            if (!$style[0]) {
                $style = $("<style id='" + gridId + "' type='text/css' rel='stylesheet' />").appendTo($("head"));//.grid.$root);
            }
        }
        $style.empty();
        var trw = $scope.totalRowWidth();
        css = "." + gridId + " > .ngViewport > .ngCanvas { width: " + trw + "px; }; " +
            "." + gridId + " > .ngViewport > .ngCanvas > .ngRow { width: " + trw + "px; }; " +        
        ...
        css += "." + gridId + " > .ngViewport > .ngCanvas > .ngRow > .sub-grid { width: " + sumWidth + "px; }; ";        
...
ng.Grid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout) {
    ...
    self.calcMaxCanvasHeight = function() {
        if (!self.config.groups.length) {
            ...
            angular.forEach(self.filteredRows, function(row){
                ht += (row.clone ? row.clone.rowHeight||self.config.rowHeight : self.config.rowHeight);
            });
            return ht;
        }
        else
        {
            return (self.rowFactory.parsedData.filter(function(e) {
                return !e[NG_HIDDEN];
    ...
    self.buildColumns = function() {
        ...
        if (columnDefs.length > 0) {
            ...
            {
                var extraColumns = self.config.extraColumns;
                if (typeof self.config.extraColumns != 'Array')
                {
                    extraColumns = [self.config.extraColumns];
                }
                indexOffset = cols.length;
                angular.forEach(extraColumns, function(colDef, i) {   
                      i += indexOffset;
                      var column = new ng.Column({
                          colDef: colDef,
                          index: i,
        ...
        if (asterisksArray.length > 0) {
            ...
            angular.forEach(asterisksArray, function(col) {
                ...
                if (col.index + 1 == numOfCols) {
                    ...
                    $scope.columns[col.index].width = Math.max($scope.columns[col.index].width - offset, 0);
                }
                if (col.visible !== false) {
...
ng.Row = function (entity, config, selectionProvider, rowIndex, $utils) {
    ...
    self.setVars = function (fromRow) {
        ...
        if(fromRow.rowHeight) self.rowHeight = fromRow.rowHeight;
...
ng.RowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
    ...
    self.renderedChange = function() {
        ...
        var top = self.renderedRange.topRow.offsetTop || 0;                
        for (var i = self.renderedRange.topRow; i < self.renderedRange.bottomRow; i++) {
            if (temp[i]) {
                temp[i].offsetTop = top;//i * grid.config.rowHeight;
                top += (temp[i].clone.rowHeight || grid.config.rowHeight);                
                rowArr.push(temp[i]);
            }
        }
    ...
    self.renderedChangeNoGroups = function () {
        ...
        for (var i = self.renderedRange.topRow; i < self.renderedRange.bottomRow; i++) {
            if (grid.filteredRows[i]) {
                ...
                grid.filteredRows[i].offsetTop = top;// i * grid.config.rowHeight;
                top += (grid.filteredRows[i].clone ? (grid.filteredRows[i].clone.rowHeight || grid.config.rowHeight) : grid.config.rowHeight);
